...
const FilterContent = ({
  ...
  <div className="bg-[#F8F5F0] lg:rounded-3xl p-6 md:p-8 lg:border border-stone-100 h-full overflow-y-auto flex flex-col">
    ...
    <div className="mb-6 md:mb-10">
      <h3 className="text-[11px] font-black uppercase tracking-widest text-primary/40 mb-4 md:mb-6 font-sans">Category</h3>
    ...
    {/* Price Range */}
    <div className="mb-6 md:mb-10">
      <h3 className="text-[11px] font-black uppercase tracking-widest text-primary/40 mb-4 md:mb-6 font-sans">Max Price</h3>
      <input
        type="range"
        min="0"
        max="5000"
    ...
    {/* Weight */}
    <div className="mb-6 md:mb-10">
      <h3 className="text-[11px] font-black uppercase tracking-widest text-primary/40 mb-4 md:mb-6 font-sans">Weight</h3>
      <div className="grid grid-cols-2 gap-2">
        {['100g', '250g', '500g', '1kg'].map(w => (
          <button
            key={w}
            onClick={() => setSelectedWeight(w)}
            className={`py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border ${selectedWeight === w
    ...
    {onApply && (
      <div className="lg:hidden mt-auto pt-6">
        <button onClick={onApply} className="w-full py-4 bg-[#331917] text-white font-black uppercase tracking-widest text-[11px] rounded-2xl active:scale-95 transition-all shadow-xl">
          Apply Filters
        </button>
      </div>
    )}
...
const Products = () => {
  ...
  useEffect(() => {
    setSelectedCategories((categorySlug || categoryId) ? [categorySlug || categoryId] : []);
  }, [categorySlug, categoryId]);

  const activeCategory = useMemo(() => {
    if (selectedCategories.length === 1) {
      return categories.find(c => c._id === selectedCategories[0] || c.slug === selectedCategories[0]);
    }
    return null;
  }, [categories, selectedCategories]);

  const seoTitle = searchTerm ? `Search results for "${searchTerm}"` : (activeCategory?.metaTitle || activeCategory?.name || "Health & Heritage Collection");
  const seoDesc = activeCategory?.metaDescription || "Browse our full collection of artisanal cookies, millet-based health powders, and traditional snacks.";
  ...
  const filteredProducts = useMemo(() => {
    let result = products.filter(p => {
      if (selectedCategories.length > 0) {
        ...
        const isMatch = selectedCategories.some(sel => sel === pCatId || sel === pCatSlug);
  ...
  return (
    <div className="pt-36 lg:pt-24 pb-16 px-4 xl:px-10 max-w-[1700px] mx-auto min-h-screen bg-[#FDFBF7]">
      ...
      <div className="lg:hidden fixed bottom-24 left-4 right-4 z-[100] flex bg-[#331917]/95 backdrop-blur-xl text-white rounded-3xl shadow-[0_20px_50px_rgba(51,25,23,0.3)] overflow-hidden border border-white/10">
        <button onClick={() => setShowFilters(true)} className="flex-1 py-4 flex items-center justify-center gap-2 border-r border-white/10 active:bg-white/10 transition-colors">
          <span className="material-symbols-outlined text-lg">filter_alt</span>
          <span className="text-[10px] font-black uppercase tracking-widest">Filters</span>
        </button>
        <button onClick={() => setShowSort(true)} className="flex-1 py-4 flex items-center justify-center gap-2 active:bg-white/10 transition-colors">
      ...
      <div className={`lg:hidden fixed inset-0 z-[110] transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] ${showFilters ? 'visible' : 'invisible'}`}>
        ...
        <div className={`absolute left-0 right-0 bottom-0 bg-[#FDFBF7] rounded-t-[2.5rem] h-[80vh] transition-transform duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] ${showFilters ? 'translate-y-0 shadow-[0_-20px_50px_rgba(0,0,0,0.2)]' : 'translate-y-full'}`}>
      ...
      {/* Mobile Sort Drawer */}
      <div className={`lg:hidden fixed inset-0 z-[110] transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] ${showSort ? 'visible' : 'invisible'}`}>
        <div className={`absolute inset-0 bg-primary/60 backdrop-blur-sm transition-opacity duration-500 ${showSort ? 'opacity-100' : 'opacity-0'}`} onClick={() => setShowSort(false)}></div>
        <div className={`absolute left-0 right-0 bottom-0 bg-[#FDFBF7] rounded-t-[2.5rem] p-8 pb-12 transition-transform duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] ${showSort ? 'translate-y-0 shadow-[0_-20px_50px_rgba(0,0,0,0.2)]' : 'translate-y-full'}`}>
          <div className="w-10 h-1 bg-stone-300 rounded-full mx-auto -mt-4 mb-6"></div>
          <h2 className="text-xl font-serif font-black text-primary italic mb-6">Sort By</h2>
          <div className="space-y-2">
            {sortOptions.map(opt => (
              ...
              >
                {opt.label}
                {sortBy === opt.value && <span className="material-symbols-outlined text-sm">check_circle</span>}
              </button>
            ))}
          </div>
          <button onClick={() => setShowSort(false)} className="w-full mt-6 py-2 text-[10px] font-black uppercase text-stone-400">Cancel</button>
        </div>
      </div>
      ...
      <nav className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-wider text-stone-400 mb-8 overflow-x-auto whitespace-nowrap hide-scrollbar">
        ...
        <span className="text-stone-900">{activeCategory?.name || "Health & Heritage Collection"}</span>
      </nav>
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <main className="flex-grow">
          ...
          <div className="hidden lg:flex bg-[#F8F5F0] rounded-2xl p-4 mb-6 justify-between items-center border border-stone-100">
            <p className="text-sm font-medium text-stone-500 italic">Found <span className="font-bold text-primary not-italic">{filteredProducts.length}</span> masterpieces</p>
            <div className="flex items-center gap-4">
              <span className="text-[10px] font-black uppercase tracking-widest text-primary/40">Sort By:</span>
              <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="bg-transparent border-none text-sm font-bold text-primary focus:ring-0 cursor-pointer">
                {sortOptions.map(o => <option key={o.value}>{o.value}</option>)}
              </select>
            </div>
          </div>
          ...
          {!loading && filteredProducts.length === 0 && (
            <div className="text-center py-24 xl:py-40 bg-white/50 rounded-[3rem] border border-stone-100/50 backdrop-blur-sm">
              <h3 className="text-2xl xl:text-4xl font-serif italic text-primary mb-4">{searchTerm ? `The item "${searchTerm}" is not available` : "Our artisanal oven is warming up..."}</h3>
              <p className="text-sm xl:text-base text-stone-400 font-medium mb-12">Try browsing our entire collection below.</p>
              <button onClick={() => { setSelectedCategories([]); setPriceRange(5000); }} className="bg-primary text-secondary-fixed px-10 py-5 rounded-2xl font-black text-[11px] uppercase tracking-[0.2em] shadow-2xl transition-all">Explore Collections</button>
            </div>
          )}
        </main>
      </div>
