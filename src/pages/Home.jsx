...
const Home = () => {
  ...
  const defaultSlides = [
    {
      imageUrl: "/assets/home-hero-cinematic.png",
      title: "Artisanal Handcrafted Cookies",
      subtitle: "DAKSHA • Freshly Baked Every Day",
    },
    {
      imageUrl: "/assets/modern-display.png",
      title: "Modern Display Collection",
      subtitle: "Curated Artisan Excellence",
    },
    {
      imageUrl: "/assets/heritage-kitchen.png",
      title: "Heritage Kitchen Recipes",
      subtitle: "Traditional Flavors Since Generations",
    },
    {
      imageUrl: "/assets/millet-powder.png",
      title: "Premium Millet Powders",
      subtitle: "Natural & Nutrient Rich Goodness",
    },
    {
      imageUrl: "/assets/premium-packaging.png",
      title: "Premium Gift Packaging",
      subtitle: "Deliver Love in Every Box",
    },
  ];
...
const DesktopCategory = ({ label, src, to }) => (
  <Link
    to={to}
    className="flex flex-col items-center gap-2 group hover:translate-y-[-4px] transition-all"
  >
...
const MobileProductCard = ({ p, onAdd, onBuy }) => (
  <div className="bg-white rounded-xl p-2.5 shadow-md flex flex-col group h-full border border-primary/5">
    <Link
      ...
      className="aspect-[3/4] rounded-xl overflow-hidden mb-3 bg-stone-50 block"
    >
      <img
    ...
    <div className="flex-1 flex flex-col px-1">
      <h4 className="font-serif font-black italic text-xs text-primary line-clamp-1 h-4">
        {p.name}
      </h4>
      <p className="font-serif font-black text-base text-tertiary italic mt-1 leading-none">
        ₹{p.variants?.[0]?.price}
      </p>
    </div>
    <div className="grid grid-cols-2 gap-2 mt-auto pt-4">
      <button
        onClick={onAdd}
        className="bg-primary text-white py-2 rounded-lg font-black text-[7px] uppercase tracking-widest"
      >
        Add
      </button>
      <button
        onClick={onBuy}
        className="bg-tertiary text-white py-2 rounded-lg font-black text-[7px] uppercase tracking-widest"
      >
        Buy
      </button>
    </div>
    ...
    <div className="aspect-[4/5] w-full rounded-2xl overflow-hidden mb-4 relative block bg-stone-50">
      ...
      <div className="absolute bottom-3 left-3 right-3 flex gap-2 z-20">
        <button
          ...
          className="flex-1 bg-primary text-white py-2.5 rounded-xl font-black text-[9px] uppercase shadow-lg hover:bg-black transition-all"
        >
          Add
        </button>
        <button
          ...
          className="flex-1 bg-tertiary text-white py-2.5 rounded-xl font-black text-[9px] uppercase shadow-lg hover:opacity-90 transition-all"
        >
          Buy Now
        </button>
      </div>
    </div>
    <h3 className="font-serif text-lg font-black text-primary italic mb-1 line-clamp-1 h-6">
      {p.name}
    </h3>
    <span className="font-serif text-xl font-black text-tertiary italic mt-auto">
      ₹{p.variants?.[0]?.price}
    </span>
  </div>
);
...
const RoundProductCard = ({ p, onBuy }) => (
  <div className="flex flex-col items-center group transition-all text-center">
    <div className="relative mb-4 block group-hover:scale-105 transition-transform duration-500">
      ...
      <button
        onClick={onBuy}
        className="absolute inset-0 bg-primary/40 opacity-0 group-hover:opacity-100 transition-opacity rounded-[2rem] flex items-center justify-center backdrop-blur-[2px]"
      >
        <span className="text-white font-black text-[9px] uppercase tracking-widest bg-primary/80 px-4 py-2 rounded-full">
          Buy
        </span>
      </button>
    </div>
    <h3 className="text-xs font-serif font-black text-primary italic leading-tight uppercase tracking-tight line-clamp-2 px-3 max-w-[150px] group-hover:text-tertiary transition-colors">
      {p.name}
    </h3>
  </div>
);
...
const ServiceCard = ({ icon, title, desc }) => (
  <div className="p-6 bg-white rounded-[2rem] border border-primary/5 shadow-sm hover:shadow-xl transition-all h-full group text-left">
    ...
    <h4 className="font-serif font-black text-xl text-primary mb-2 italic">
      {title}
    </h4>
    <p className="text-stone-500 text-sm leading-relaxed font-sans italic">
      {desc}
    </p>
  </div>
);
...
const TestimonialCard = ({ name, quote, loc, dark }) => (
  <div
    className={`p-6 rounded-[2rem] relative shadow-lg transition-all hover:translate-y-[-4px] ${dark ? "bg-primary text-white" : "bg-white text-primary border border-primary/5"}`}
  >
    <span className="material-symbols-outlined text-5xl absolute top-4 right-4 opacity-10">
      format_quote
    </span>
    <p className="text-lg italic font-serif leading-relaxed mb-5 font-bold relative z-10">
      "{quote}"
    </p>
    <div>
      ...
      <p className="text-[10px] uppercase font-black tracking-widest opacity-60 italic">
        {loc}
      </p>
    </div>
  </div>
...
const MarqueeCarousel = ({
  ...
  return (
    <div className={`marquee-container ${className}`}>
      <div
        className={`marquee-track ${gap} animate-marquee`}
