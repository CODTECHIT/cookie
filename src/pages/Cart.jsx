...
const Cart = () => {
  const { 
    cartItems, removeFromCart, updateQty, subtotal, clearCart,
    shippingCost, isShippingFree, threshold
  } = useCart();
  const { user } = useUser();
  ...
  const savings = cartItems.reduce((acc, item) => {
    const itemSaving = (item.oldPrice || 0) > (item.price || 0) 
  ...
  const handleCheckout = async () => {
    ...
    try {
        ...
        const { data: orderResponse } = await axios.post(`${API_URL}/orders`, orderData, {
        ...
        const { data: rzpOrderResponse } = await axios.post(`${API_URL}/payments/razorpay-order`, {
            amount: finalTotal,
        ...
        const options = {
            ...
            description: 'Handcrafted Cookies & Millets',
    ...
    <div className="pt-32 md:pt-44 pb-24 px-4 xl:px-10 max-w-[1700px] mx-auto min-h-screen bg-[#FDFBF7]">
      ...
      <div className="grid lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="lg:col-span-2 space-y-3">
          {cartItems.map(item => (
            <div key={`${item.id}-${item.variantId}`} className="bg-white rounded-2xl md:rounded-[2rem] p-3 md:p-4 flex gap-4 md:gap-6 items-center border border-stone-100 shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="flex-grow flex flex-col sm:flex-row justify-between w-full h-full items-start">
                <div className="flex flex-col h-full min-w-0">
                  <h3 className="text-sm md:text-xl font-bold text-primary mb-0.5 truncate">{item.title}</h3>
                  ...
                  <div className="flex items-center gap-4 md:gap-6 bg-stone-50 p-1 md:p-1.5 rounded-lg md:rounded-xl border border-stone-200 w-fit mt-auto">
                    <button onClick={() => updateQty(item.id, item.variantId, -1)} className="w-6 h-6 md:w-8 md:h-8 flex items-center justify-center text-stone-400 hover:text-primary"><span className="material-symbols-outlined text-[14px]">remove</span></button>
                    <span className="text-[11px] md:text-sm font-bold text-primary">{item.qty}</span>
                    <button onClick={() => updateQty(item.id, item.variantId, 1)} className="w-6 h-6 md:w-8 md:h-8 flex items-center justify-center text-stone-400 hover:text-primary"><span className="material-symbols-outlined text-[14px]">add</span></button>
                  </div>
                </div>

                <div className="flex flex-col items-end h-full justify-between sm:pt-2 ml-auto">
                  <span className="text-base md:text-2xl font-bold text-primary">₹{item.price * item.qty}</span>
                  <button
          ...
          <Link to="/products" className="inline-flex items-center gap-2 text-sm font-bold text-[#D4A017] hover:gap-4 transition-all pt-6">
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-[2.5rem] p-8 border border-stone-100 shadow-sm">
             <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-xl font-serif font-black text-primary italic">Shipping Details</h2>
             </div>
             ...
             <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                   ...
                   <div className="space-y-1">
                      <label className="text-[10px] font-black uppercase text-stone-400 ml-2">Phone</label>
                      <input 
                        type="tel" 
                        required
                        className="w-full bg-stone-50 border border-stone-100 rounded-xl px-4 py-3 text-xs font-bold outline-none focus:ring-1 focus:ring-primary"
                ...
                <div className="space-y-1">
                   <label className="text-[10px] font-black uppercase text-stone-400 ml-2">Flat / House / Street</label>
                   <input 
                     type="text" 
                     required
                     className="w-full bg-stone-50 border border-stone-100 rounded-xl px-4 py-3 text-xs font-bold outline-none focus:ring-1 focus:ring-primary"
                ...
                <div className="grid grid-cols-2 gap-3">
                   <div className="space-y-1">
                      <label className="text-[10px] font-black uppercase text-stone-400 ml-2">Pincode</label>
                      <input 
                        type="text" 
                        required
                        maxLength={6}
                   ...
                   <div className="space-y-1">
                      <label className="text-[10px] font-black uppercase text-stone-400 ml-2">City</label>
                      <input 
                        type="text" 
                        required
                        className="w-full bg-stone-50 border border-stone-100 rounded-xl px-4 py-3 text-xs font-bold outline-none focus:ring-1 focus:ring-primary"
                ...
                <div className="space-y-1">
                   <label className="text-[10px] font-black uppercase text-stone-400 ml-2">Landmark (Optional)</label>
                   <input 
                     type="text" 
                     className="w-full bg-stone-50 border border-stone-100 rounded-xl px-4 py-3 text-xs font-bold outline-none focus:ring-1 focus:ring-primary"
          ...
          <div className="bg-[#F8F5F0] rounded-[2.5rem] p-6 border border-stone-100">
            ...
            <div className="mb-8 bg-white p-4 rounded-2xl border border-stone-200 flex items-center gap-3">
              <Truck className={isShippingFree ? "text-emerald-500" : "text-primary"} size={18} />
              <div className="flex flex-col">
                <span className={`text-[10px] font-black uppercase ${isShippingFree ? "text-emerald-600" : "text-primary"}`}>
                  {isShippingFree ? 'Free Shipping Applied' : 'Artisanal Delivery'}
                </span>
            ...
            <h2 className="text-2xl font-serif font-black text-primary italic mb-6">Price Details</h2>
            ...
            <div className="space-y-3 mb-6 text-sm font-medium">
              ...
              <div className="flex justify-between text-stone-500">
                ...
                <span className={`font-bold ${isShippingFree ? "text-emerald-600" : "text-primary"}`}>
              ...
              <div className="flex justify-between text-[#D4A017]">
                ...
                <span className="font-bold">- ₹{savings + currentDiscount}</span>
              </div>
            </div>
            ...
            <div className="border-t border-stone-200 pt-8 mb-10">
              <div className="flex justify-between items-end">
                ...
                <span className="text-4xl font-serif font-black text-primary italic leading-none">₹{finalTotal}</span>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center p-4 bg-white rounded-2xl border border-stone-100 text-center">
              ...
              <span className="text-[9px] font-bold text-primary uppercase tracking-widest text-center">100% Secure & Encrypted Payments</span>
            </div>
          </div>
            ...
            {appliedCoupon ? (
                <div className="p-6 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-[10px] font-black text-stone-400 uppercase">Coupon Applied!</p>
                            <p className="text-sm font-black text-primary">{appliedCoupon.code}</p>
                        </div>
                    </div>
                    <button onClick={removeCoupon} className="text-[10px] font-black uppercase text-red-500 hover:scale-105 transition-all">Remove</button>
                </div>
            ) : (
