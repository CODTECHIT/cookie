...
const ReportsManagement = () => {
    ...
    try {
      ...
      if (data.success && data.data.length > 0) {
        ...
        setSalesReport({
          ...
          avgOrderValue: report.avgOrderValue || 0,
  ...
  const handleDownloadPdf = async () => {
    try {
      ...
      const response = await axios.get(`${API_URL}/reports/export/bi-pdf`, {
        params: { period: reportType },
        headers: { Authorization: `Bearer ${token}` },
        responseType: "blob",
      });
      ...
      link.href = url;
      link.download = `business-intelligence-${reportType}-${new Date().toISOString().split("T")[0]}.pdf`;
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 font-bold mt-2 text-sm uppercase tracking-widest leading-tight">
            Advanced sales analytics and profit optimization reports.
          </p>
        </div>
      ...
      {loading ? (
        <div className="h-60 flex items-center justify-center p-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="bg-white p-7 rounded-[32px] shadow-sm border border-gray-100 flex flex-col justify-between group hover:shadow-xl transition-all border-b-4 border-b-blue-500">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest text-emerald-500 bg-emerald-50 px-2.5 py-1 rounded-full">
                  +12.5%
                </span>
              </div>
              <div>
            ...
            <div className="bg-white p-7 rounded-[32px] shadow-sm border border-gray-100 flex flex-col justify-between group hover:shadow-xl transition-all border-b-4 border-b-emerald-500 text-emerald-900">
              ...
              <div>
                ...
                <h3 className="text-2xl font-black text-emerald-700 tracking-tight leading-none">
                  ₹{calculateProfit()?.toLocaleString()}
                </h3>
                <p className="text-[10px] text-emerald-600/50 font-bold mt-2 uppercase tracking-tighter">
                  70% Margin analysis
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-white p-8 rounded-[40px] shadow-sm border border-gray-100 space-y-6 flex flex-col items-center justify-center text-center relative overflow-hidden group">
              ...
              <p className="text-gray-400 text-xs font-bold uppercase tracking-widest max-w-xs">
                Data visualization module is aggregating {reportType} data
                points for high-fidelity charting.
              </p>
              ...
              <div className="w-full flex justify-between space-x-1 mt-8 h-20 items-end px-4">
                {[40, 70, 45, 90, 65, 80, 50, 95, 30, 85].map((h, i) => (
                  <div
                    key={i}
                    className="flex-1 bg-primary/20 rounded-t-lg group-hover:bg-primary transition-all duration-500"
                    style={{ height: `${h}%` }}
                  ></div>
                ))}
              </div>
            ...
            <div className="bg-white p-8 rounded-[40px] shadow-sm border border-gray-100">
              <div className="flex items-center justify-between mb-8">
                <h2 className="text-sm font-black text-gray-900 uppercase tracking-widest flex items-center">
                  <Award className="w-5 h-5 mr-2 text-primary" /> Top Performing
                  Inventory
                </h2>
              ...
              <div className="space-y-4">
                {bestSellers.length > 0 ? (
                  bestSellers.map((item, idx) => (
                    <div
                      key={idx}
                      className="flex items-center space-x-4 p-4 rounded-3xl bg-gray-50/50 hover:bg-white hover:shadow-lg hover:border-primary/20 transition-all border border-transparent group"
                    >
      ...
      <div className="bg-gray-900 p-10 rounded-[48px] shadow-2xl relative overflow-hidden group">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8 text-center md:text-left">
          <div>
            ...
            <p className="text-white/40 text-sm font-bold uppercase tracking-widest max-w-sm leading-relaxed mb-8">
              Detailed profit analysis and demographic breakdown reports are
              generating based on high-order transaction data.
            </p>
            <div className="flex flex-wrap gap-4 items-center justify-center md:justify-start">
              <div className="px-6 py-3 bg-white/10 rounded-2xl border border-white/10">
                <p className="text-[10px] text-white/40 font-black uppercase tracking-[0.2em] mb-1">
                  Avg Order Val.
                </p>
          ...
          <button
            ...
            className="bg-primary text-white p-12 rounded-[40px] font-black text-xs uppercase tracking-[0.3em] flex flex-col items-center hover:scale-105 transition-all shadow-2xl shadow-primary/40 group active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed"
          >
            <Calendar className="w-8 h-8 mb-4 opacity-50 group-hover:opacity-100 transition-all" />
            {downloadingPdf ? "Preparing" : "Download"}
            <br />
            {downloadingPdf ? "PDF..." : "Full PDF"}
          </button>
        </div>
      </div>
    </div>
