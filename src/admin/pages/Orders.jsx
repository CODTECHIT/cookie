...
const OrdersManagement = () => {
  ...
  const handleUpdateTracking = async (e) => {
    ...
    try {
      const { data } = await axios.patch(
        `${API_URL}/orders/${selectedOrder._id}/tracking`,
        trackingData,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-gray-50/50 border-b border-gray-100">
                  ...
                  <th className="px-6 py-4 text-xs font-bold text-gray-500 uppercase tracking-wider text-right uppercase">
                    Actions
                  </th>
                </tr>
              </thead>
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <p className="font-medium text-gray-800 text-sm">
                        {o.customerSnapshot?.name || o.customerId?.name}
                      </p>
                      <p className="text-xs text-gray-400">
                        {o.customerSnapshot?.phone || o.customerId?.phone}
                      </p>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <div
                        className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold border ${getStatusColor(o.status)} uppercase tracking-tighter`}
                      >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span
                          className={`text-[10px] font-bold uppercase ${o.paymentStatus === "Paid" ? "text-emerald-600" : "text-amber-600"}`}
                        >
                          {o.paymentStatus}
                        </span>
            ...
            <div className="flex-1 overflow-y-auto p-8 custom-scrollbar border-r border-gray-100 bg-gray-50/30">
              <div className="flex items-center justify-between mb-8">
                <div>
                  ...
                  <p className="text-sm font-bold text-primary mt-2 uppercase tracking-widest">
                    {selectedOrder.orderNumber}
                  </p>
                </div>
                <div
                  className={`px-4 py-1.5 rounded-full text-xs font-bold border shadow-sm ${getStatusColor(selectedOrder.status)}`}
                >
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-3">
                ...
                {selectedOrder.discount > 0 && (
                  <div className="flex justify-between text-sm text-emerald-600 font-bold">
                    <span>Discount Applied ({selectedOrder.couponCode})</span>
                    <span>- ₹{selectedOrder.discount}</span>
                  </div>
                )}
                <div className="pt-3 border-t border-dashed border-gray-200 flex justify-between items-center text-lg">
                  <span className="text-gray-900 font-black uppercase tracking-tight">
                    Total Amount
                  </span>
            ...
            <div className="w-full md:w-[350px] overflow-y-auto p-8 border-l border-gray-100 space-y-8 bg-white z-10">
              ...
              <div className="space-y-4">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 font-medium">
                      Customer ID:{" "}
                      {selectedOrder.customerId?._id?.substring(0, 8) ||
                        "GUEST"}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-start space-x-3">
                  ...
                  <div className="text-xs text-gray-600 font-medium leading-relaxed">
                    ...
                    {selectedOrder.customerSnapshot?.address?.city},{" "}
                    {selectedOrder.customerSnapshot?.address?.state}
                    <br />
                    <span className="font-black text-gray-900 tracking-widest">
                      {selectedOrder.customerSnapshot?.address?.pincode}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="p-4 rounded-2xl bg-gray-50 border border-gray-100 space-y-3">
                  ...
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span
                      className={`font-black uppercase tracking-tighter ${selectedOrder.paymentStatus === "Paid" ? "text-emerald-600" : "text-amber-600"}`}
                    >
              ...
              <div className="space-y-3">
                ...
                <p className="text-[10px] text-gray-400 font-bold uppercase mb-4">
                  Update progress to keep customer informed
                </p>
                ...
                <div className="space-y-2">
                  {[
                    ...
                    <button
                      key={status}
                      disabled={
                        selectedOrder.status === status || statusLoading
                      }
                    ...
                    >
                      ...
                      {!statusLoading && selectedOrder.status !== status && (
              ...
              <button
                onClick={() => setShowDetailModal(false)}
                className="w-full py-4 text-sm font-black text-gray-500 uppercase tracking-widest hover:bg-gray-50 rounded-2xl border-2 border-dashed border-gray-100 mt-4 transition-all active:scale-95"
              >
                Close Details
              </button>
            </div>
