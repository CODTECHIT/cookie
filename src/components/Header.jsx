  ...
  const navLinks = [
    ...
    { 
      ...
      dropdown: categories.length > 0 
        ? categories.map(c => ({ name: c.name, path: `/category/${c.slug || c._id}` }))
  ...
  return (
    <>
      ...
      {activeCoupon && (
        <div className="bg-[#331917] text-[#fed255] py-2 px-4 text-center text-[10px] font-black uppercase tracking-[0.3em] z-[60] relative min-h-[32px] flex items-center justify-center">
           <span className="italic flex items-center justify-center gap-2">
             ...
             Limited Offer: Use code {activeCoupon.code} to get {activeCoupon.discountType === 'percentage' ? `${activeCoupon.discountValue}%` : `₹${activeCoupon.discountValue}`} off! • {activeCoupon.description}
           </span>
        </div>
      )}
      ...
      <div className="hidden lg:block">
        {/* Fixed Header */}
        <header className={`fixed ${(activeCoupon && !scrolled) ? 'top-8' : 'top-0'} w-full z-50 transition-all duration-500 ease-in-out ${scrolled ? 'bg-white shadow-[0_10px_30px_-10px_rgba(51,25,23,0.1)]' : 'bg-white/80'} backdrop-blur-xl border-b border-primary/5`}>
          <div className="max-w-[1700px] mx-auto px-10 py-5 flex justify-between items-center transition-all duration-500">
            ...
            <Link to="/" className="flex items-center gap-3 group">
              <div className="w-10 h-10 rounded-xl overflow-hidden shadow-inner">
                <img src="/logo.png" alt="Daksha Logo" className="w-full h-full object-contain" />
              </div>
            ...
            <nav className="flex items-center gap-8">
              {navLinks.map((link) => (
                <div key={link.name} className="relative group/nav">
                  <Link
                    to={link.path}
                    className={`text-[11px] font-black uppercase tracking-[0.2em] transition-all relative py-2 flex items-center gap-1.5 ${location.pathname === link.path ? 'text-primary' : 'text-stone-500 hover:text-primary'
                      }`}
                  >
                    {link.name}
                    {link.dropdown && <span className="material-symbols-outlined text-sm font-black transition-transform group-hover/nav:rotate-180">expand_more</span>}
                    <span className={`absolute -bottom-1 left-0 h-0.5 bg-[#D4A017] transition-all duration-500 ${location.pathname === link.path ? 'w-full' : 'w-0 group-hover/nav:w-full'}`}></span>
                  </Link>
                    ...
                    <div className="absolute top-full left-1/2 -translate-x-1/2 pt-4 opacity-0 translate-y-2 invisible group-hover/nav:opacity-100 group-hover/nav:translate-y-0 group-hover/nav:visible transition-all duration-300 z-50">
                      <div className="bg-white rounded-2xl shadow-[0_20px_60px_-15px_rgba(51,25,23,0.2)] border border-primary/5 p-4 min-w-[240px] grid grid-cols-1 gap-1">
                        <div className="px-4 py-2 border-b border-stone-50 mb-2">
                          <span className="text-[10px] font-black uppercase tracking-[0.3em] text-stone-300">Discover Collections</span>
                        </div>
                        {link.dropdown.map(sub => (
                          ...
                          >
                            {sub.name}
                            <span className="material-symbols-outlined text-xs opacity-0 group-hover/sub:opacity-100 translate-x-[-10px] group-hover/sub:translate-x-0 transition-all">arrow_forward</span>
                          </Link>
                        ))}
                      </div>
                    </div>
            ...
            <div className="flex items-center gap-6">
              <form onSubmit={handleSearchSubmit} className="flex items-center bg-stone-100/50 rounded-xl px-4 py-2 border border-primary/5 focus-within:border-primary/20 focus-within:bg-white transition-all shadow-sm group/search">
                <span className="material-symbols-outlined text-stone-600 text-lg mr-2 group-focus-within/search:text-primary transition-colors cursor-pointer" onClick={handleSearchSubmit}>search</span>
                <input
                  type="text"
              ...
              <div className="flex items-center gap-2">
                ...
                {user ? (
                  <div className="flex items-center gap-4 ml-2 group/user relative">
                    <Link to="/my-orders" className="flex items-center gap-3 pl-4 border-l border-primary/10 group-hover/user:translate-x-[-2px] transition-all hover:opacity-80">
                      ...
                      <div className="flex flex-col">
                        <span className="text-[9px] font-black uppercase text-stone-400 leading-none mb-0.5">Welcome back,</span>
                        <span className="text-xs xl:text-[13px] font-black text-primary truncate max-w-[100px] tracking-tight">{user.name.split(' ')[0]}</span>
                      </div>
                    </Link>
                    <button
                      onClick={logout}
                  ...
                  <Link to="/login" className="ml-2 bg-primary text-secondary-fixed px-8 py-3.5 rounded-xl font-black text-[10px] uppercase tracking-[0.2em] hover:shadow-2xl hover:translate-y-[-2px] transition-all active:scale-95 shadow-xl shadow-primary/20 block">
                    Login / Sign up
                  </Link>
                )}
              </div>
      ...
      <div className="lg:hidden">
        ...
        <div className={activeCoupon ? "h-20" : "h-14"} />
        <header className={`fixed top-0 w-full z-50 transition-all duration-300 bg-primary text-white ${scrolled ? 'shadow-lg' : ''}`}>
          ...
          {activeCoupon && (
            <div className="bg-[#331917] text-[#fed255] py-2 px-4 text-center text-[9px] font-black uppercase tracking-[0.2em] relative min-h-[32px] flex items-center justify-center border-b border-white/5">
               <span className="italic flex items-center justify-center gap-2">
                 ...
                 {activeCoupon.code}: {activeCoupon.discountType === 'percentage' ? `${activeCoupon.discountValue}%` : `₹${activeCoupon.discountValue}`} OFF!
               </span>
            </div>
          )}
          ...
          <div className="flex items-center justify-between px-4 h-14">
            <div className="flex items-center gap-4">
              <button onClick={() => setIsMenuOpen(true)} className="material-symbols-outlined text-xl">menu</button>
              <Link to="/" className="font-serif text-xl font-black italic tracking-tight uppercase">Daksha</Link>
            </div>
            <div className="flex items-center gap-4">
              {user ? (
                <Link to="/my-orders" className="w-8 h-8 rounded-full bg-[#fed255] flex items-center justify-center text-primary font-black text-[10px] shadow-lg">
                  {user.name.charAt(0)}
                </Link>
              ) : (
                <Link to="/login" className="text-[10px] font-black uppercase tracking-widest bg-white/10 px-3 py-1.5 rounded-lg border border-white/10">Login</Link>
              )}
            </div>
          </div>
          {/* Secondary Search Bar (Flipkart Style) */}
          <div className="px-3 pb-3">
            <form onSubmit={handleSearchSubmit} className="bg-white rounded-xl h-10 flex items-center px-4 gap-3 shadow-inner">
            ...
            <div className="p-6 h-full flex flex-col">
              ...
              <nav className="flex flex-col gap-4">
                {navLinks.map((link) => (
                  <Link
                    ...
                    className={`flex items-center justify-between p-4 rounded-2xl transition-all ${location.pathname === link.path ? 'bg-primary text-secondary-fixed' : 'text-primary/60 hover:bg-primary/5'
                      }`}
                  >
              ...
              <div className="mt-auto pt-10 border-t border-primary/5 space-y-6">
                <div className="flex items-center gap-4 text-primary/60">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest">{settings?.phone || "+91 9704254959"}</span>
                </div>
                {user ? (
                  <div className="space-y-3">
                    <div className="p-4 bg-primary/5 rounded-2xl border border-primary/10">
                      <p className="text-[9px] font-black uppercase text-primary/40 leading-none mb-1">Signed in as</p>
