...
export const sendEmail = async (options) => {
  if (!process.env.EMAIL_USER || !process.env.EMAIL_PASS) {
    throw new Error(
      `Email credentials missing in .env: USER=${!!process.env.EMAIL_USER}, PASS=${!!process.env.EMAIL_PASS}`,
    );
  }
  ...
  const mailOptions = {
    from: `"dakshacookiesmillets" <${process.env.EMAIL_USER}>`,
    to: options.email,
    ...
    html:
      ...
      <div style="font-family: inherit; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; padding: 20px; color: #331917;">
        <div style="background-color: #FDFBF7; padding: 40px; border-radius: 20px; border: 1px solid #eee; text-align: center;">
          <h1 style="color: #331917; font-size: 24px; margin-bottom: 20px;">dakshacookiesmillets</h1>
  ...
  try {
    ...
    console.log(
      `✅ Email sent successfully to ${options.email}. Response: ${info.response}`,
    );
  } catch (err) {
